...
export function CursorFog() {
  ...
  return <div className="cursor-mist" aria-hidden="true"><span /><span /><span /></div>;
}
...
export function Hero() {
  ...
  useEffect(() => {
    const revealTimer = window.setTimeout(() => setRevealed(true), 5000);
  ...
  return (
    <section id="home" className="hero">
      ...
      <div className="container hero-grid">
        <div className={`hero-copy-wrap ${revealed && !transitioning ? 'hero-copy-visible' : 'hero-copy-hidden'}`}>
      ...
      <div className="container order-grid">
        ...
        <form onSubmit={submit} className="order-card">
          <div className="form-grid">
            ...
            <label><span>Delivery date</span><input required type="date" value={form.date} onChange={(event) => update('date', event.target.value)} /></label>
...
export function Gallery() {
  const gallery = [
    ...
    ['/images/hero-gautam.png', 'A.K. Gautam Traders business portrait'],
  ];
  return (
    <section id="gallery" className="section-pad gallery-section">
      <div className="container">
        ...
        <div className="gallery-grid">
          {gallery.map(([src, alt], index) => <div key={src} className={`gallery-item gallery-${index + 1}`}><Image src={src} alt={alt} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" /></div>)}
        </div>
      </div>
    </section>
  );
